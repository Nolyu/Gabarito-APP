...
def compute_layout(config, folha_id):
    ...
    # ─────────────────────────────────────────────────────────────
    D_MAX = 8.2 * mm          # diâmetro confortável (o que você aprovou)
...
def ler_barcode(corrigida, layout, W, H):
    ...
    for cell in bc["cells"]:
        ...
        if regiao.size == 0:
            return None
        # Compara a célula com o papel logo à direita dela, em vez de usar um
        # brilho fixo: com sombra atravessando a folha, metade do código ficaria
        # "escura" por causa da luz, não por estar pintada.
        larg = x2 - x1
        px1 = min(W - 1, x2 + int(larg * 0.6))
        px2 = min(W, px1 + max(3, larg))
        volta = corrigida[max(0, y1):y2, px1:px2]
        media_cel = float(regiao.mean())
        if volta.size:
            ref = max(float(volta.mean()), 40.0)
            bits.append(1 if (ref - media_cel) / ref >= 0.45 else 0)
        else:
            bits.append(1 if (1 - media_cel / 255) >= 0.5 else 0)
    if len(bits) != 14:
        return None
...
def processar_scan(foto_bytes, layout, gabarito_raw, folha_esperada=None):
    ...
    # Se a folha TEM código de barras mas ele saiu ilegível, o alinhamento
    # provavelmente está errado (cantos mal escolhidos). Melhor recusar do que
    # entregar uma nota calculada em cima de posições erradas.
    if layout.get("barcode") and id_barcode is None:
        raise ValueError("Não consegui conferir a folha — mostre-a inteira, "
