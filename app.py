...
def decidir_marcadas(intensidades):
    """Limiar absoluto + relativo (espelha a lógica do app)."""
    vals = sorted(intensidades.values())
    if not vals:
        return []
    max_v = vals[-1]
    piso = vals[len(vals) // 2]  # mediana ≈ bolinhas vazias
    ABS_MIN = 0.42
    if max_v < ABS_MIN:
        return []
    return [a for a, v in intensidades.items() if v >= ABS_MIN and (v - piso) >= 0.18]
...
def ler_questoes(corrigida, layout, W, H):
    ...
    for q_num, alts in layout["questoes"].items():
        ...
        marcadas = decidir_marcadas(intensidades)
        resultados[q_num] = {
            "marcadas": marcadas,
