...
def api_pacote_completo():
    ...
    # folhas disponíveis (próprias + compartilhadas) para criar quiz pelo app
    folhas_rows = db.execute("SELECT id, nome, config FROM folhas WHERE user_id=? ORDER BY id DESC", (uid,)).fetchall()
    comp_rows = db.execute("""
        SELECT f.id, f.nome, f.config FROM folhas_compartilhadas fc
