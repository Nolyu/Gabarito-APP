...
def compute_layout(config, folha_id):
    ...
    # Faixas de densidade, como o ZipGrade: poucas questões ganham bolinhas
    # grandes; muitas questões usam bolinhas menores e mais colunas.
    # (raio, espaçamento entre bolinhas, altura da linha, máx. colunas)
    FAIXAS = [
        (4.1 * mm, 11.5 * mm, 11.5 * mm, 2),   # confortável
        (3.4 * mm,  9.4 * mm,  9.4 * mm, 3),   # média
        (3.0 * mm,  8.4 * mm,  7.8 * mm, 3),   # compacta
    ]

    escolhida = None
    for r_b, sp, rh, max_c in FAIXAS:
        max_rows = int(altura_disp / rh)
        if max_rows < 1:
            continue
        # cada coluna: recuo (5mm) + até o centro da 1ª bolinha (7mm)
        # + vãos entre bolinhas + raio final + folga (2mm)
        lc = 5 * mm + 7 * mm + (max_labels - 1) * sp + r_b + 2 * mm
        max_c_larg = max(1, int(CONTENT_W // lc))
        n_col = min(math.ceil(nq / max_rows), max_c_larg, max_c)
        n_col = max(1, n_col)
        if math.ceil(nq / n_col) <= max_rows:
            escolhida = (r_b, sp, rh, n_col, max_rows)
            break
    if not escolhida:
        # calcula o teto real com a faixa mais compacta
        r_b, sp, rh, max_c = FAIXAS[-1]
        max_rows = max(1, int(altura_disp / rh))
        lc = 5 * mm + 7 * mm + (max_labels - 1) * sp + r_b + 2 * mm
        teto = max_rows * min(max_c, max(1, int(CONTENT_W // lc)))
        return None, (f"Muitas questões para uma página: cabem no máximo "
                      f"{teto} com essa configuração.")

    r_b, sp, rh, n_col, max_rows = escolhida
    layout["bubble_radius_pt"] = r_b   # a leitura usa este raio
    por_col = (nq + n_col - 1) // n_col
    col_w = CONTENT_W / n_col

    for col in range(n_col):
        x_base = CONTENT_X + col * col_w + 5 * mm
...
def desenhar_folha(c, config, layout, folha_id, cinza=0.62, versao_marcada=None):
    ...
    if "versao_bolhas" in layout:
        ...
        for L, pos in layout["versao_bolhas"].items():
            ...
            if versao_marcada and L == versao_marcada:
                ...
                c.circle(cxp, cyp, BUBBLE_R, fill=1, stroke=1)
            else:
                c.setStrokeColorRGB(0.55, 0.55, 0.55)
    ...
    r_q = layout.get("bubble_radius_pt", BUBBLE_R)
    if r_q >= 3.9 * mm:
        f_num, f_letra, dy_letra = 15, 9.5, 3.2
    elif r_q >= 3.2 * mm:
        f_num, f_letra, dy_letra = 13, 8.5, 2.9
    else:
        f_num, f_letra, dy_letra = 11.5, 7.5, 2.5
...
def desenhar_relatorio_aluno(c, quiz_nome, scan, layout):
    ...
    for rec in recortes[:3]:
        if not rec.get("img"):
            continue
        try:
            img = ImageReader(io.BytesIO(base64.b64decode(rec["img"])))
            iw, ih = img.getSize()
            alt = 11 * mm
    ...
    if scan["debug_img"]:
        try:
            img = ImageReader(io.BytesIO(base64.b64decode(scan["debug_img"])))
            iw, ih = img.getSize()
    ...
    for i, r in enumerate(rows, 1):
        ...
        for j, idx in enumerate(escolhidas):
            ...
            if not img_b64:
                continue
            try:
                img = ImageReader(io.BytesIO(base64.b64decode(img_b64)))
                iw, ih = img.getSize()
                alt = 9 * mm
