...
def encontrar_marcadores(cinza):
    ...
    for grupo in itertools.combinations(por_area, 4):
        ...
        por_y = sorted(pts, key=lambda p: p[1])
        topo = sorted(por_y[:2], key=lambda p: p[0])
        base = sorted(por_y[2:], key=lambda p: p[0])
        ordem = [topo[0], topo[1], base[0], base[1]]
        ...
        if sim_w < 0.75 or sim_h < 0.75:
            continue
        razao = ((h_esq + h_dir) / 2) / ((w_top + w_bot) / 2)
        if razao < 0.75 or razao > 2.2:
            continue
...
def preenchimento_local(img, cx, cy, raio):
    ...
    return fill, max(0.0, (brilho_papel - brilho_miolo) / 255)
