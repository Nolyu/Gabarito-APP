...
def api_pacote_completo():
    ...
    folhas_rows = db.execute("SELECT id, nome, config, layout FROM folhas WHERE user_id=? ORDER BY id DESC", (uid,)).fetchall()
    comp_rows = db.execute("""
        SELECT f.id, f.nome, f.config, f.layout FROM folhas_compartilhadas fc
